...
func TestHandler_getMetric(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:       "Ok",
			inputBody:  `{"id": "Alloc", "metricType": "gauge", "value": 12345.6}`,
			id:         "Alloc",
			metricType: "gauge",
			value:      12345.6,
			...
			want: want{
				code:        200,
				response:    `{"id": "Alloc", "metricType": "gauge", "value": 12345.6}`,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			request := httptest.NewRequest(http.MethodPost, "/value/", bytes.NewBufferString(tt.inputBody))
